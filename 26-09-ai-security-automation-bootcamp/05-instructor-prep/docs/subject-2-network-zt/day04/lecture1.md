...
|----------------|----------------------|------|
| **VPN(브이피엔)** | 공용망 위에 만든 암호화된 통로 | 도로 위 전용터널 |
| **터널(tunnel, 터널)** | 내용을 감싸 못 보게 하는 통로 | 밀봉 컨테이너 |
| **암호화(encryption)** | 내용을 알아볼 수 없게 바꾸기 | 자물쇠 채우기 |
| **스니핑(sniffing, 스니핑)** | 오가는 데이터를 몰래 엿보기 | 도청 |
| **Site-to-Site(사이트투사이트)** | 네트워크↔네트워크 통째 연결 | 건물 간 구름다리 |
| **Remote Access(리모트 액세스)** | 개인 PC → 회사망 접속 | 집에서 쓰는 열쇠 |
| **VPN 게이트웨이** | 터널을 유지하는 양쪽 장비 | 터널 입구 관리소 |
| **클라이언트(client, 클라이언트)** | 내 PC에 까는 접속 프로그램 | 출입 앱 |
| **측면 이동(lateral movement)** | 내부에서 옆 자원으로 옮겨 다니기 | 집 안 방 옮겨다니기 |
...
!!! question "확인질문"
    ...
    **A.** 아이디·비밀번호·업무 자료가 **암호도 안 된 채** 공용 인터넷(특히 카페 와이파이)을 지나갑니다. 그 사이에 누가 **몰래 엿보거나 가로챌(스니핑)** 수 있어요. VPN 터널로 감싸면, 가로채도 안의 내용은 못 봅니다.
...
!!! question "확인질문"
    ...
    **A.** **네트워크와 네트워크**(예: 본사망 ↔ 지사망)를 통째로 잇습니다. 개인이 프로그램을 켜는 게 아니라, 양쪽에 있는 **VPN 장비**가 알아서 터널을 계속 유지해요.
...
!!! question "확인질문"
    ...
    **A.** **Remote Access VPN**입니다. 개인이 자기 PC에서 프로그램을 켜고 로그인해 회사망에 들어오는 방식이거든요. Site-to-Site는 지점 전체를 잇는 거라 개인용이 아닙니다.
...
!!! question "확인질문"
    ...
    **A.** 계정 하나만 뺏겨도 공격자가 **내부를 자유롭게 돌아다니며(측면 이동)** 중요한 자료에 닿을 수 있습니다. "터널 안에 들어왔으니 믿는다"라서, 내부에서 다시 확인하지 않기 때문이에요. → 그래서 자원마다 매번 확인하는 Zero Trust가 필요합니다.
