...
|----------------|----------------------|------|
| **DNS(디엔에스)** | 도메인 이름을 IP로 바꿔주는 시스템 | 전화번호부 |
| **도메인(domain, 도메인)** | 사람이 외우는 인터넷 이름 | 가게 상호 |
| **Root 서버(루트)** | 조회의 최상위 출발점 | 대표 안내데스크 |
| **TLD(티엘디)** | .com·.kr 등 꼬리표 담당 서버 | 지역 안내소 |
| **Authoritative 서버(권위)** | 그 도메인의 진짜 답을 가진 서버 | 그 집 주인 |
| **리졸버(resolver, 리졸버)** | 대신 여기저기 물어봐 주는 심부름꾼 | 비서 |
| **재귀 질의(recursive)** | "알아서 다 찾아와" 한 번에 맡기기 | 심부름 시키기 |
| **반복 질의(iterative)** | 서버들에 차례로 물어보기 | 단계별 문의 |
| **캐싱(caching, 캐싱)** | 찾은 답을 잠시 저장해 재사용 | 냉장고에 보관 |
| **레코드(record, 레코드)** | DNS에 저장된 한 줄 항목 | 전화번호부 한 줄 |
| **A 레코드** | 도메인 → IPv4 주소 | 이름 → 번호 |
| **CNAME(씨네임)** | 다른 이름의 별명 | 별칭·닉네임 |
| **MX 레코드(엠엑스)** | 그 도메인의 메일 서버 | 우편 담당 주소 |
| **TTL(티티엘)** | 답을 얼마나 저장해둘지 시간 | 유통기한 |
...
!!! question "확인질문"
    ...
    **A.** 사이트마다 **숫자로 된 IP 주소**(예: `93.184.216.34`)를 직접 외워야 합니다. 도메인 이름 대신 이런 숫자를 다 외우는 건 사실상 불가능하죠. DNS가 그 수고를 대신 해줍니다.
...
!!! question "확인질문"
    ...
    **A.** 이론상 새 조회의 출발점이 막혀 큰 혼란이 됩니다. 하지만 실제로는 잘 안 멈춰요.

    - Root 서버가 **전 세계에 여러 대로 나뉘어(이중화)** 있고,
    - 이미 찾아둔 답이 여기저기 **저장(캐싱)**돼 있기 때문입니다.

    그래서 한두 대 죽어도 인터넷이 바로 멈추진 않습니다. (그래서 Root는 아주 튼튼하게 운영됩니다)
...
!!! question "확인질문"
    ...
    **A.** 그 복잡한 심부름을 **리졸버가 대신** 해주기 때문입니다. 리졸버는 한 번 찾은 답을 **저장(캐싱)**해 둬서, 다음 사람은 훨씬 빨리 받아요. 모든 PC가 매번 Root부터 다 뒤지면 느리고, Root 서버에도 부담이 몰립니다.
...
!!! question "확인질문"
    ...
    **A.** **CNAME**을 씁니다. `www.example.com`을 `example.com`의 **별명**으로 걸어두면, 나중에 IP가 바뀌어도 한 곳(example.com)만 고치면 둘 다 자동으로 따라옵니다.
