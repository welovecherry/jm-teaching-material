...
|----------------|----------------------|------|
| **nslookup(엔에스룩업)** | 윈도우에서 DNS를 조회하는 명령 | 번호 조회 |
| **dig(딕)** | 맥·리눅스의 DNS 조회 명령 | 번호 조회 |
| **리졸버(resolver, 리졸버)** | 우리 대신 DNS 서버들을 오가며 답을 찾아주는 심부름꾼 | 대신 찾아주는 비서 |
| **dig +trace(딕 트레이스)** | Root부터 전체 조회 경로 보기 | 경로 추적 |
| **캐싱(caching, 캐싱)** | 찾은 답을 잠시 저장 | 냉장고 보관 |
| **TTL(티티엘)** | 그 저장이 유지되는 시간 | 유통기한 |
| **전파 지연(propagation)** | 바꾼 값이 퍼지는 데 걸리는 시간 | 소문 퍼지는 시간 |
| **DNS 스푸핑(스푸핑)** | 가짜 DNS 답으로 가짜 사이트 유도 | 전화번호부 위조 |
| **캐시 포이즈닝(포이즈닝)** | 조작된 답을 캐시에 심기 | 냉장고에 상한 음식 |
| **피싱(phishing, 피싱)** | 진짜인 척 속여 정보 빼내기 | 가짜 은행 창구 |
| **DGA(디지에이)** | 악성코드가 무작위 도메인 대량 생성 | 매번 바뀌는 대포폰 |
| **C2(씨투)** | 공격자가 감염 PC를 조종하는 서버 | 범인 본부 |
| **NXDOMAIN(엔엑스도메인)** | "그런 도메인 없음" 응답 | 없는 번호 |
...
!!! question "확인질문"
    ...
    **A.** 도메인 하나를 찾아가는 **전체 경로**를 Root → TLD → 최종 서버까지 **한 단계씩** 보여줍니다. "어디에 물어보고, 또 어디로 넘어가는지"를 눈으로 따라갈 수 있는 도구예요.
...
!!! question "확인질문"
    ...
    **A.** 그 사용자(또는 그 사람이 쓰는 리졸버)에 **예전 IP가 아직 저장(캐싱)**돼 있고, 그 **유효기간(TTL)이 안 끝나서** 옛 주소를 계속 쓰기 때문입니다. 이런 "전파 지연" 때문에, IP를 바꾸기 전엔 보통 TTL을 미리 짧게 줄여둡니다.
...
!!! question "확인질문"
    ...
    **A.** **DNS 스푸핑(주소 조작)**을 의심합니다. 주소는 제대로 쳤는데, 누군가 DNS 답을 조작해 **가짜 IP로 연결**시켰을 수 있어요. (자물쇠·인증서 경고가 함께 뜨는지도 같이 확인합니다.)
...
!!! question "확인질문"
    ...
    **A.** **악성코드 감염**을 의심합니다. 보통 사람은 읽을 수 있는 도메인을 가끔 조회하지, 뜻 없는 무작위 문자열을 초당 수십 개씩 조회하지 않거든요. 악성코드가 공격자 서버를 몰래 찾으려고 그러는 경우라, 그 PC를 격리하고 조사합니다.
