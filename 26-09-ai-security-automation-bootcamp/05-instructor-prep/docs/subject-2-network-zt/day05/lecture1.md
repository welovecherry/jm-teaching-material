...
|----------------|----------------------|------|
| **클라우드(cloud, 클라우드)** | 남의 서버를 빌려 쓰는 것 | 남의 집 빌리기 |
| **IaaS(아이아스)** | 서버(인프라)만 빌리기 | 빈 땅 임대 |
| **PaaS(파스)** | 개발 환경까지 갖춘 걸 빌리기 | 골조 지어진 집 |
| **SaaS(사스)** | 완성된 서비스를 그냥 쓰기 | 완제품 아파트 |
| **책임 공유 모델** | 어디까지 내가·공급자가 책임지나 | 임대 계약 범위 |
| **프록시(proxy, 프록시)** | 나가는 트래픽을 대신 중계·검사 | 대리 창구 |
| **가시성(visibility)** | 무슨 일이 있는지 보이는 정도 | CCTV 유무 |
| **DMZ(디엠지)** | 외부 공개 서버를 두는 완충 구역 | 현관 응접실 |
| **외부/내부 방화벽** | DMZ 앞뒤로 두는 두 겹의 벽 | 대문·중문 |
| **망 구성도(network diagram)** | 장비·구간을 그린 지도 | 건물 배치도 |
| **draw.io(드로우아이오)** | 무료 다이어그램 작성 도구 | 그림판 |
| **VPN 터널** | 지점 간 암호화 연결선 | 전용 구름다리 |
...
!!! question "확인질문"
    ...
    **A.** **SaaS**입니다. 완성된 서비스를 그대로 갖다 쓰고, 서버·프로그램 관리는 전부 만든 회사가 합니다. (우리 회사는 계정·권한만 챙기면 됩니다.)
...
!!! question "확인질문"
    ...
    **A.** 거의 파악하기 어렵습니다. 그 접속이 **회사 방화벽·프록시를 안 거치니까**, 회사에 기록(로그)이 남지 않거든요. 이렇게 "안 보이게 되는" 문제 때문에 Zero Trust 같은 새 방식이 필요해집니다.
...
!!! question "확인질문"
    ...
    **A.** DMZ가 없으면 웹서버가 내부망과 **같은 방**에 있는 셈입니다. 그래서 웹서버를 뚫은 공격자가 **막는 벽 없이** 곧장 DB·사무망으로 넘어갈 수 있어요. DMZ는 그 사이에 방화벽을 하나 더 둬서, 피해가 번지는 걸 막습니다.
...
!!! question "확인질문"
    ...
    **A.** 사내 PC → 사내망 → **방화벽/프록시** → 인터넷 → SaaS 서버 순서로 갑니다. 핵심은 나갈 때 **방화벽/프록시를 꼭 거친다**는 점이에요. 바로 여기서 통제하고 기록을 남깁니다.
