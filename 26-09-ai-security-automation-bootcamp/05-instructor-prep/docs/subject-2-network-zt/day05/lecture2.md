...
|----------------|----------------------|------|
| **Latency(레이턴시)** | 목적지까지 걸리는 시간 | 편지 도착 일수 |
| **RTT(왕복시간)** | 갔다가 돌아오는 데 걸린 시간 | 편지+답장 시간 |
| **Jitter(지터)** | 지연이 들쭉날쭉한 정도 | 배송일이 매번 다름 |
| **Packet Loss(패킷 로스)** | 가다가 사라진 패킷 비율 | 분실된 택배 |
| **ping(핑)** | 목적지에 신호 보내 왕복시간 재기 | "거기 있어요?" |
| **프록시(proxy, 프록시)** | 나가는 트래픽을 대신 중계·검사하는 중간 서버 | 대리 창구 |
| **traceroute / tracert** | 거쳐 가는 구간별 지연 보기 | 경로 추적 |
| **홉(hop, 홉)** | 거쳐 가는 라우터 한 곳 | 갈아타는 역 |
| **혼잡(congestion)** | 길이 막혀 느려지는 상태 | 교통 체증 |
| **큐 대기(queue)** | 장비에서 순서를 기다림 | 줄 서기 |
| **ISP(아이에스피)** | 인터넷 회선을 제공하는 통신사 | 도로 관리청 |
| **실시간성(real-time)** | "지금 이 순간"이 중요한 성질 | 생방송 |
| **대역폭(bandwidth)** | 한 번에 보낼 수 있는 양 | 도로 차선 수 |
...
!!! question "확인질문"
    ...
    **A.** 전체적으로는 빠른데도 **소리·화면이 자꾸 끊기고 튀는** 느낌이 듭니다. 조각들이 들쭉날쭉 도착해서 재생 박자가 흔들리기 때문이에요. 평균 속도만 보면 놓치는 문제입니다.
...
!!! question "확인질문"
    ...
    **A.** 일시적인 **혼잡(길 막힘)**이나 어느 한 구간이 잠깐 바빴던 걸 의심합니다. 딱 한 번이면 그냥 순간적인 튐일 수 있어요. 하지만 **자꾸 반복되면 그 구간에 계속 문제**가 있다는 신호라, traceroute로 더 확인합니다.
...
!!! question "확인질문"
    ...
    **A.** 그 홉 전까지는 멀쩡하다가 거기서 갑자기 느려졌다면, **바로 그 앞 구간에서 막힘·문제**가 생겼을 가능성이 큽니다. "어디까지 괜찮고, 어디서 나빠졌나"로 문제 구간을 좁혀가는 거예요.
...
!!! question "확인질문"
    ...
    **A.** 그 **느린 구간을 담당하는 장비/경로**를 봅니다. 사내 구간이면 우리 라우터·방화벽·프록시를, 바깥 구간이면 통신사(ISP) 문제일 수 있어요. 사내가 의심되면 **방화벽/프록시 기록**으로 지연·차단을 확인합니다.
