...
|----------------|----------------------|------|
| **TCP(티씨피)** | 잘 받았는지 확인하며 확실히 보내는 방식 | 등기우편 |
| **UDP(유디피)** | 확인 없이 빠르게 보내는 방식 | 일반우편 |
| **3-way Handshake(쓰리웨이 핸드셰이크)** | 연결 전 세 번 주고받는 인사 | "여보세요?" 3번 |
| **SYN(신)** | "연결하고 싶어요" 첫 신호 | 노크 |
| **ACK(액)** | "잘 받았어요" 확인 신호 | 대답 |
| **SYN Flood(신 플러드)** | SYN만 잔뜩 보내 서버 자리를 채우는 공격 | 예약만 하고 안 오기 |
| **IP 헤더(아이피 헤더)** | 패킷 겉봉에 적힌 정보(출발지·목적지 등) | 택배 송장 |
| **TTL(티티엘)** | 패킷의 남은 수명(0이면 폐기) | 유통기한 |
| **패킷(packet, 패킷)** | 데이터를 잘게 나눈 한 덩어리 | 택배 상자 |
| **포트(port, 포트)** | 한 컴퓨터 안 프로그램 구분 번호 | 건물 안 호실 |
| **tracert(트레이서트)** | 목적지까지 거치는 길을 보여주는 명령 | 경로 추적기 |
| **Wireshark(와이어샤크)** | 오가는 패킷을 들여다보는 도구 | 택배 X-레이 |
| **필터(filter, 필터)** | 보고 싶은 패킷만 걸러내는 조건 | 검색어 |
