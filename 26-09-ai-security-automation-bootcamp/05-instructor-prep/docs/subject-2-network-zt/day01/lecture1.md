...
| **프로토콜(protocol, 프로토콜)** | 통신할 때 서로 지키기로 한 약속·규칙 | 편지 주소 쓰는 규칙 |
| **비트(bit, 비트)** | 컴퓨터가 쓰는 가장 작은 단위(0 또는 1) | 스위치 하나(켜짐/꺼짐) |
| **패킷(packet, 패킷)** | 데이터를 잘게 나눠 담은 한 덩어리(주로 3계층) | 택배 상자 하나 |
| **프레임(frame, 프레임)** | 2계층에서 데이터를 담는 봉투(겉면에 MAC 적힘) | 이름표 붙은 택배 상자 |
| **캡슐화(encapsulation, 인캡슐레이션)** | 층마다 봉투(헤더)를 하나씩 덧씌우기 | 편지→봉투→택배상자 |
| **헤더(header, 헤더)** | 봉투 겉면에 적는 주소·정보 | 택배 송장 |
| **MAC 주소(맥)** | 기기(랜카드)마다 평생 붙어 있는 고유 번호 | 주민등록번호 |
| **랜카드 / NIC(엔아이씨)** | 컴퓨터를 네트워크에 연결해주는 부품(랜선 꽂는 곳) | 통신용 귀·입 |
| **OUI(오유아이)** | MAC 앞 3칸 = 만든 회사 표시 | 제조사 마크 |
| **IP 주소(아이피)** | 지금 접속한 위치를 가리키는 주소 | 지금 사는 집 주소 |
| **ARP(아르프)** | IP로 상대의 MAC을 알아내는 절차 | "그 집 이름표가 뭐죠?" |
| **게이트웨이(gateway, 게이트웨이)** | 우리 동네에서 밖으로 나가는 정문 | 아파트 정문 |
| **포트(port, 포트)** | 한 컴퓨터 안에서 어느 프로그램인지 구분하는 번호 | 건물 안 "몇 호실" |
| **허브(hub, 허브)** | 신호를 그냥 모두에게 퍼뜨리는 1계층 장비 | 확성기 |
| **스위치(switch, 스위치)** | MAC 보고 같은 네트워크 안 정확히 전달(2계층) | 아파트 경비실 |
| **라우터(router, 라우터)** | IP 보고 다른 네트워크로 넘김(3계층) | 지역 간 택배 허브 |
...
먼저 낯선 단어 하나. **랜카드(NIC, 엔아이씨)**는 **컴퓨터가 네트워크에 연결되게 해주는 부품**입니다. 노트북·PC에서 **랜선을 꽂는 그 구멍(또는 Wi-Fi 안테나)**이 바로 랜카드예요. 이 부품마다 세상에 하나뿐인 번호가 찍혀 있는데, 그게 아래 **MAC 주소**입니다.

- **주소:** **MAC(맥) 주소** — **랜카드마다** 공장에서 찍혀 나오는 고유번호(기기의 이름표).
...
먼저 단어 하나 더. **프레임(frame, 프레임)**은 **2계층에서 데이터를 담는 봉투 한 개**입니다.

!!! example "쉬운 비유 — 프레임 = 2계층의 택배 상자"
    앞에서 데이터를 잘게 나눈 덩어리를 **패킷**이라 했죠. 사실 층마다 부르는 이름이 다릅니다: **3계층에선 '패킷', 2계층에선 '프레임'**. 둘 다 "데이터 한 덩어리(봉투)"인데, 프레임 겉면엔 ==출발지 MAC과 목적지 MAC== 이 적혀 있습니다.

이제 스위치 이야기. 스위치엔 랜선을 꽂는 구멍(**포트**)이 여러 개 있는데, 처음엔 **어느 기기가 어느 포트에 꽂혔는지 모릅니다.** 그래서 프레임이 도착할 때마다 **"이 프레임의 출발지 MAC은 이 포트에서 왔구나"**를 표(MAC 주소 테이블)에 적어두며 **스스로 배웁니다.** 아래가 그 과정입니다:

```mermaid
flowchart LR
    A["프레임(데이터 봉투) 도착<br/>→ 출발지 MAC을 '이 포트'로 기록"] --> B{목적지 MAC을<br/>테이블에서 아나?}
    B -->|모름| C[모든 포트로 일단 뿌림<br/>flooding]
    B -->|앎| D[그 포트로만 콕 전달]
    C --> E[응답이 오면<br/>그 MAC 위치도 학습]
    E --> B
