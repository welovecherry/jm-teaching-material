...
| 시간 | 소주제 | 핵심 한 줄 |
|------|--------|-----------|
| 00-25분 | IPv4 주소 구조 (+IPv6 맛보기) | 32비트를 8비트씩 4묶음, 공인 vs 사설 |
...
|----------------|----------------------|------|
| **비트(bit, 비트)** | 컴퓨터가 쓰는 가장 작은 단위(0 또는 1) | 스위치 하나(켜짐/꺼짐) |
| **IPv4(아이피브이포)** | 32비트로 된 인터넷 주소 체계 | 4칸짜리 주소 |
| **IPv6(아이피브이식스)** | 128비트로 늘린 차세대 주소 체계 | 8칸짜리 주소 |
| **옥텟(octet, 옥텟)** | IP를 8비트씩 나눈 한 묶음 | 주소의 한 칸 |
| **16진수(hex, 헥스)** | 0~9 다음 a~f까지 쓰는 표기법 | 숫자에 알파벳까지 동원 |
| **듀얼 스택(dual stack)** | IPv4와 IPv6를 동시에 쓰는 방식 | 2개 국어 병기 |
| **서브네팅(subnetting, 서브네팅)** | 큰 IP 대역을 작은 서브넷으로 나누는 작업 | 큰 땅을 필지로 나누기 |
...
### 🌍 곁눈질 — IPv6, 주소가 모자라서 나온 후속 체계

IPv4는 32비트니까 주소를 **최대 2³² = 약 43억 개**밖에 만들 수 없습니다. 전 세계 사람이 스마트폰·노트북·IoT 기기를 여러 대씩 쓰는 지금은 **턱없이 부족합니다**(= 앞서 본 **IP 고갈**). 그래서 주소 길이를 확 늘린 **IPv6**가 나왔습니다.

**IPv6는 128비트**입니다. 표기법도 달라서, **16비트씩 8묶음**을 **16진수**로 쓰고 **콜론(`:`)**으로 잇습니다.

```text
IPv4 (32비트)  : 192.168.0.1
                 └ 8비트씩 4묶음, 10진수, 점(.)으로 구분

IPv6 (128비트) : 2001:0db8:0000:0000:0000:ff00:0042:8329
                 └ 16비트씩 8묶음, 16진수, 콜론(:)으로 구분
```

주소가 너무 길어서 **줄여 쓰는 규칙 2개**가 있습니다. 이 둘만 알면 실무에서 만나는 IPv6는 읽힙니다.

| 규칙 | 내용 | 예시 |
|------|------|------|
| ① 앞자리 0 생략 | 각 묶음의 **맨 앞 0들**을 지움 | `0db8` → `db8`, `0042` → `42` |
| ② 연속된 0 묶음은 `::` | **0으로만 된 묶음이 연달아** 있으면 `::`로 압축 (한 주소에 **딱 한 번만**) | `0000:0000:0000` → `::` |

```text
2001:0db8:0000:0000:0000:ff00:0042:8329   ← 원래 주소
2001:db8:0:0:0:ff00:42:8329               ← ① 앞자리 0 생략
2001:db8::ff00:42:8329                    ← ② 연속 0을 :: 로 압축 (최종형)
```

!!! example "쉬운 비유 — 43억 개도 모자란다는 감각"
    43억 개라면 넉넉해 보이지만, **지금 지구 인구가 약 80억 명**입니다. 한 사람당 하나씩 나눠주는 것조차 안 됩니다.
    게다가 요즘은 한 사람이 스마트폰·노트북에 더해 CCTV·도어락·자동차까지 주소를 요구하죠. IPv4는 **애초에 인구수도 못 따라가는 크기**였던 셈입니다.
    IPv6는 여기서 자릿수를 다른 차원으로 올립니다. **지구에 있는 모든 모래알에 주소를 하나씩 붙이고도 한참 남는** 수준이라, 다시 모자랄 걱정은 하지 않아도 됩니다.
...
|---|---|---|
| 길이 | 32비트 | **128비트** |
| 개수 | 약 43억 개 | 약 3.4×10³⁸ 개 (사실상 무한) |
| 표기 | 10진수 + 점(`.`) | 16진수 + 콜론(`:`) |
| 예 | `192.168.0.1` | `2001:db8::1` |
| NAT | **거의 필수**(주소 부족) | **불필요**(주소가 남아돎) |

!!! warning "학생이 헷갈리는 지점"
    **"IPv6가 나왔으면 IPv4는 이제 안 쓰나요?"** → 아닙니다. **둘을 동시에 쓰는 중**입니다.

    기존 장비·서비스가 워낙 많아 한 번에 갈아탈 수 없어서, 대부분의 기기는 IPv4 주소와 IPv6 주소를 **둘 다 가지고** 상황에 맞게 씁니다. 이것을 **듀얼 스택(dual stack)**이라고 합니다. 그래서 실무에서는 **여전히 IPv4 서브네팅이 핵심 기술**이고(오늘 배우는 내용), IPv6는 "같이 돌아가는 또 하나의 체계"로 알아두면 됩니다.

!!! tip "실무 팁 — 보안 관점에서 IPv6가 던지는 숙제"
    IPv6는 주소가 남아돌아 **NAT를 쓸 필요가 없습니다.** 편해 보이지만 보안 관점에서는 **주의 신호**입니다.

    - NAT가 없으면 내부 기기가 **공인 주소를 직접 갖고 인터넷에 노출**됩니다. 즉 뒤에서 배울 "NAT = 첫 번째 방어선"이라는 **가림막이 사라집니다.**
    - 그래서 IPv6 환경에서는 **방화벽 정책이 훨씬 더 중요**해집니다. "NAT 뒤에 있으니 안전하다"는 생각이 통하지 않죠.
    - 이것이 **제로트러스트(Zero Trust)**의 출발점과 같은 이야기입니다. *위치(내부망이냐)로 신뢰하지 말고, 매번 검증하라.* (이번 과목의 큰 줄기입니다)

<div class="quiz">
<p class="quiz-q"><span class="tag">퀴즈</span><b>IPv6 주소 <code>2001:0db8:0000:0000:0000:0000:0000:0001</code>을 올바르게 축약한 것은?</b></p>
<button class="quiz-opt">2001:db8:0:1</button>
<button class="quiz-opt" data-correct>2001:db8::1</button>
<button class="quiz-opt">2001::db8::1</button>
<button class="quiz-opt">2001.db8.0.1</button>
<div class="quiz-explain"><b>정답: 2번.</b> 앞자리 0을 지워 <code>0db8</code>→<code>db8</code>, 연속된 0 묶음을 <code>::</code>로 압축해 <code>2001:db8::1</code>이 됩니다. 1번은 8묶음을 복원할 수 없어 틀렸고, 3번은 <code>::</code>를 <b>두 번</b> 써서 위반(어디까지가 0인지 알 수 없음), 4번은 IPv4식 점 표기입니다.</div>
<button class="quiz-retry">다시 풀기</button>
</div>

!!! question "확인질문"
    **Q. IPv6는 주소가 사실상 무한한데, 왜 회사 네트워크에서 서브네팅을 계속 배워야 할까요?**

    **A.** 두 가지 이유입니다.

    - **아직 IPv4가 현역**입니다. 듀얼 스택으로 병행 중이라 사내망 설계는 대부분 IPv4 기준으로 합니다.
    - **서브네팅의 목적이 '주소 절약'만이 아닙니다.** 부서·용도별로 대역을 갈라두면 **트래픽을 분리하고 접근을 통제**할 수 있습니다. 주소가 무한해도 이 목적은 그대로라, IPv6에서도 대역을 나눠 씁니다.
...
- **IPv6**: 128비트(16비트×8, 16진수+콜론), 주소 고갈의 근본 해법. 지금은 IPv4와 **듀얼 스택**으로 병행
...
- [ ] IPv6가 왜 나왔는지(43억 개 고갈)와 `::` 축약 규칙을 설명한다
- [ ] "IPv6가 IPv4를 대체했나?"에 **듀얼 스택**으로 답한다
...
*[IPv4]: Internet Protocol version 4 — 32비트로 된 현재 주류 IP 주소 체계
*[IPv6]: Internet Protocol version 6 — 128비트로 늘린 차세대 IP 주소 체계
