...
|----------------|----------------------|------|
| **라우팅(routing, 라우팅)** | 목적지까지 어느 길로 보낼지 정하기 | 내비게이션 |
| **라우팅 테이블** | 라우터가 보는 경로표 | 길 안내표 |
| **다음 홉(next hop, 넥스트 홉)** | 바로 다음에 넘길 곳 | 다음 갈아탈 역 |
| **기본 게이트웨이** | 아는 길 없을 때 내보내는 정문 | 아파트 정문 |
| **기본 경로(0.0.0.0/0)** | "나머지 전부는 여기로" 규칙 | 기타 우편함 |
| **정적 라우팅(static)** | 관리자가 손으로 경로 입력 | 종이 지도 |
| **동적 라우팅(dynamic)** | 라우터끼리 경로를 자동 교환 | 실시간 내비 |
| **백본망(backbone, 백본)** | 통신사의 대형 중심 회선 | 고속도로 본선 |
| **스위치(switch)** | 같은 네트워크 안에서 전달 | 아파트 경비실 |
| **VLAN(브이랜)** | 한 스위치를 논리적으로 나눔 | 사무실 칸막이 |
| **DMZ(디엠지)** | 외부 공개 서버를 두는 완충 구역 | 현관 응접실 |
| **브로드캐스트(broadcast)** | 같은 동네 전체에 보내는 신호 | 마을 방송 |
...
!!! question "확인질문"
    ...
    **A.** **기본 게이트웨이(정문)**로 보냅니다. 라우터는 아는 길이 없으면 "일단 정문으로 내보내자"는 규칙(`0.0.0.0/0`)이 있거든요. 그래서 인터넷으로 나가는 대부분의 트래픽이 이 정문으로 모입니다.
...
!!! question "확인질문"
    ...
    **A.** **대형 통신사 백본망**입니다. 길이 수없이 많고 자주 바뀌는 데다, 한 곳이 고장 나면 자동으로 우회해야 해서 사람이 일일이 손으로 못 넣어요. 반대로 작은 지사는 길이 몇 개뿐이라, 손으로 적어두는 정적 방식으로 충분합니다.
...
!!! question "확인질문"
    ...
    **A.** 게스트(외부 손님)가 **사내망과 같은 네트워크에 들어와** 내부 PC·서버·공유폴더를 넘볼 수 있습니다. VLAN으로 갈라놓으면, 게스트는 인터넷만 쓰고 사내망에는 아예 닿지 못하게 격리됩니다.
...
!!! question "확인질문"
    ...
    **A.** DMZ에 있는 서버는 **외부에 공개돼 있어** 공격받기 쉽습니다. 그래서 그 서버가 뚫리더라도 ==진짜 중요한 내부망(안방)까지는 바로 못 넘어가게== 사이에 방화벽 벽을 하나 더 둔 것입니다.
