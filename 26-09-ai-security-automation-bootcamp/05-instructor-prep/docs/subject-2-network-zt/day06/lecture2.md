...
!!! question "확인질문 · 나의 답"
    **Q. PDP가 '거부'라고 판단했는데 PEP가 이를 무시하고 통과시킨다면 어떤 문제가 생길까요?**
    A. 아무리 똑똑하게 판단해도 **집행이 안 되면 무의미**합니다. 거부해야 할 접근이 그대로 통과돼 **보안 통제가 붕괴**합니다. 그래서 PEP는 PDP 결정을 반드시 그대로 집행해야 합니다.
...
!!! question "확인질문 · 나의 답"
    **Q. 이 흐름에서 만약 기기 보안 상태가 기준 미달이면 PDP는 어떤 결정을 내려야 할까요?**
    A. **거부하거나, 추가 인증을 요구하거나, 제한된(읽기 전용 등) 접근만 허용**해야 합니다. 신원(ID)이 맞아도 기기(Device) 축에서 걸리면 그대로 통과시키면 안 됩니다.
...
!!! question "확인질문 · 나의 답"
    **Q. 신입사원에게 처음부터 모든 시스템 접근권한을 주는 것과, 필요할 때마다 요청받아 부여하는 것 — ZT 철학엔 어느 쪽이 맞을까요?**
    A. **필요할 때마다 요청받아 부여**하는 쪽입니다. ZT는 default deny(기본 거부)에서 출발해 최소권한만 허용합니다. 처음부터 다 주면 계정 탈취·오남용 시 피해가 커집니다.
...
!!! question "확인질문 · 나의 답"
    **Q. 로그인 이후 세션을 끊지 않고 계속 유지하면, 중간에 계정이 탈취돼도 이를 어떻게 알아챌 수 있을까요?**
    A. **지속적 검증**으로 세션 중 상태 변화(갑작스런 위치·기기·행동 변화)를 감지해, 이상하면 재인증을 요구하거나 세션을 끊습니다. 로그인 때 한 번만 보면 탈취 후를 못 잡습니다.
...
!!! question "확인질문 · 나의 답"
    **Q. 이런 점수 체계가 있으면, 고객사에게 '보안 수준이 어느 정도'라고 설명하기 더 쉬워질까요?**
    A. 네. 막연한 "안전합니다" 대신 **"5개 항목 중 3개 충족, 60점, MFA가 취약"**처럼 근거와 개선점을 구체적으로 제시할 수 있습니다(7일차 문서화와 연결).
