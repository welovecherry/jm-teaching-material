...
|----------------|----------------------|------|
| **PDP(피디피)** | 접근 허용 여부를 판단하는 두뇌 | 심사관 |
| **PEP(펩)** | 그 판단을 집행하는 문지기 | 개찰구 |
| **정책(policy, 폴리시)** | 판단의 기준이 되는 규칙 | 사내 규정 |
| **최소권한(least privilege)** | 꼭 필요한 만큼만 권한 부여 | 필요한 열쇠만 |
| **기본 거부(default deny)** | 처음엔 아무 권한도 없음에서 시작 | 백지에서 시작 |
| **지속적 검증** | 로그인 후에도 계속 다시 확인 | 순찰 계속 돌기 |
| **세션(session, 세션)** | 로그인해서 이어지는 접속 상태 | 통화 중인 상태 |
| **재인증(re-authentication)** | 중간에 다시 본인 확인 요구 | 중간 검문 |
| **위험 기반 인증** | 위험도에 따라 검증 강도 조절 | 경보 단계 |
| **스코어링(scoring, 스코어링)** | 보안 수준을 점수로 매기기 | 건강 점수 |
| **성숙도(maturity)** | 얼마나 잘 갖췄는지 수준 | 레벨 |
