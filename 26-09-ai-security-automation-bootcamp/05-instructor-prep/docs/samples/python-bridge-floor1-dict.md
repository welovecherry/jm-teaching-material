...
!!! note "📓 연습장 노트북 (같이 여세요)"
    이 교재는 **읽고 이해하는** 용도입니다. 손으로 직접 쳐보는 건 짝꿍 노트북에서 합니다.

    → **[`python-bridge-floor1-practice.ipynb`](python-bridge-floor1-practice.ipynb)** (문제 번호가 이 교재와 똑같습니다)

    노트북엔 **문제와 빈 칸만** 있습니다. 막히면 이 교재의 `▶ 정답 보기`를 펼치세요.
