# 강의1 · 계정·권한·역할·그룹과 RBAC 구현 (오전, 총 120분)
...
| 70-95분 | 역할 부여/회수 함수 & JSON 분리 | 데이터를 코드 밖 파일로 빼기 |
| 95-120분 | 조회 CLI & 정리 | 명령 한 줄로 권한 확인 |

## 이 교시에 나오는 어려운 용어

| 용어 (한글발음) | 한 줄 뜻 (아주 쉽게) | 비유 |
|----------------|----------------------|------|
| **접근통제(Access Control, 액세스 컨트롤)** | 누가 무엇에 접근할 수 있는지 정하고 지키기 | 건물 출입 통제 |
| **인증(Authentication, 어센티케이션)** | "너 진짜 그 사람 맞아?" 신원 확인 | 신분증 검사 |
| **인가(Authorization, 오소라이제이션)** | "이건 열 수 있어?" 권한 확인 | 출입증 등급 확인 |
| **RBAC(알백)** | 역할을 중간에 두고 권한을 관리하는 방식 | 직책에 권한을 묶기 |
| **계정(Account, 어카운트)** | 누구 (사용자 이름) | 사원 한 명 |
| **권한(Permission, 퍼미션)** | 무엇을 할 수 있나 | 특정 방 열쇠 |
| **역할(Role, 롤)** | 권한을 묶은 꾸러미 | 직책(영업담당자 등) |
| **그룹(Group, 그룹)** | 계정을 묶은 것 | 부서(영업팀 등) |
| **최소권한(least privilege, 리스트 프리빌리지)** | 꼭 필요한 권한만 주기 | 필요한 방 열쇠만 |
| **dict(딕셔너리)** | 이름표로 값을 찾는 파이썬 상자 | 이름-번호 전화번호부 |
| **list(리스트)** | 순서 있는 목록 | 장보기 목록 |
| **`.get(키, 기본값)`** | 키가 없으면 에러 대신 기본값 | 없으면 "빈손"으로 돌려줌 |
| **PDP/PEP(피디피/펩)** | 접근을 판단(PDP)하고 집행(PEP)하는 두 부품 | 심사관 / 개찰구 |
...
### 🔬 깊이 보기 — 인증(로그인)과 인가(권한), 순서로 이해하기

둘은 **순서가 다른 두 관문**입니다. 로그인(인증)을 통과해도, 자원마다 권한(인가)을 또 확인합니다.

```mermaid
sequenceDiagram
    participant U as 사용자
    participant Auth as 인증(로그인)
    participant App as 자원(인사시스템)
    U->>Auth: 아이디/비밀번호 (+ 문자인증)
    Auth-->>U: ✅ 본인 확인 (인증 통과)
    U->>App: 인사시스템 열기 요청
    App->>App: 인가 확인 — 이 사람 권한 있나?
    App-->>U: 권한 없음 → ❌ 거부
```

**포인트:** 로그인은 "**너 누구야?**"(인증), 자원 접근은 "**너 이거 봐도 돼?**"(인가)입니다. 로그인에 성공해도 특정 문서는 막힐 수 있어요. 오늘 배우는 RBAC이 바로 이 **'인가' 부분**을 코드로 만드는 것입니다.
...
### 🔬 깊이 보기 — `d[키]` vs `d.get(키, 기본값)`, 뭐가 다른가

파이썬에서 딕셔너리 값을 꺼내는 방법이 두 가지인데, **없는 키를 만났을 때** 완전히 다릅니다.

```mermaid
flowchart TD
    K[없는 키로 조회] --> A{어떤 방법으로?}
    A -->|"d[키]"| E["💥 KeyError 발생<br/>프로그램 멈춤"]
    A -->|"d.get(키, 기본값)"| S["🟢 기본값 반환<br/>계속 진행"]
...
- **`d['없는사람']`** → 곧바로 **`KeyError`(에러)**로 멈춥니다.
- **`d.get('없는사람', [])`** → 에러 없이 **빈 목록 `[]`**을 돌려줍니다.

그래서 `has_permission()`은 `.get(user, [])`을 써서, **등록 안 된 사용자도 에러 없이 "권한 없음(False)"**으로 안전하게 처리합니다. "모르는 사람은 막는다(기본 거부)"라는 보안 사고와도 딱 맞습니다.
...
!!! success "✍️ 지금 직접 쳐보기 (5분) — 학생에게 줄 시간"
    노트북(주피터) 셀에 위 코드를 **직접 쳐서** 실행해 봅니다.

    1. `roles`에 `'견적서_작성'` 권한이 있으니, `has_permission('kim01', '견적서_작성', roles, user_roles)`은 뭐가 나올까요? → 예측하고 실행.
    2. `user_roles`에 `'lee02': ['개발자']`를 **한 줄 추가**하고, `roles`에도 `'개발자': ['코드_배포']`를 넣어 봅니다.
    3. `has_permission('lee02', '고객정보_조회', ...)`은? → **예측 후 실행**해 이유를 말해 봅니다.

    > 🎓 강사 팁: 학생이 값을 바꿔 실행하며 "아, 역할에 없으면 False구나"를 **스스로 발견**하게 하는 게 핵심입니다.
...
## ⏱️ 70-95분 · 역할 부여/회수 함수와 JSON 분리

!!! abstract "이 블록을 마치면"
    ✔ 역할을 주고 뺏는 함수를 이해하고 ✔ ==데이터를 코드에서 떼어 JSON 파일로== 관리하는 이유를 안다

### 역할 부여 함수 — `assign_role()`

```python
def assign_role(user, role, user_roles):
    user_roles.setdefault(user, [])   # ① 이 사용자 칸이 없으면 빈 목록으로 만들기
    if role not in user_roles[user]:  # ② 아직 그 역할이 없을 때만
        user_roles[user].append(role) # ③ 역할을 추가
```

| 줄 | 하는 일 | 왜 |
|:--:|---------|-----|
| **①** | `setdefault`로 사용자 칸을 **없으면 새로** 만듦 | 없는 사용자에게도 에러 없이 부여 |
| **②** | 이미 가진 역할인지 확인 | **중복 추가 방지** |
| **③** | 역할 목록에 추가 | 실제 부여 |

!!! example "🎓 강사 뷰 · 회수는 반대로"
    역할을 뺏는 `revoke_role()`은 `user_roles[user].remove(role)`로 만듭니다.
    단, 없는 역할을 빼려 하면 에러가 나므로 **`if role in ...`으로 먼저 확인**하는 걸 짚어 주세요.

### 🔬 깊이 보기 — 왜 데이터를 JSON 파일로 빼는가

지금까지 `roles`·`user_roles`를 **코드 안에** 적었습니다. 실무에선 이걸 **JSON 파일로 분리**합니다.

```mermaid
flowchart LR
    subgraph 나쁨["❌ 코드 안에 하드코딩"]
      C["rbac.py<br/>roles = {...}<br/>권한 바꾸려면 코드 수정"]
    end
    subgraph 좋음["✅ 데이터 분리"]
      P["rbac.py<br/>(로직만)"] --> J["roles.json<br/>(권한 데이터)"]
    end
    classDef bad fill:#e03131,stroke:#b02525,color:#fff
    classDef ok fill:#2f9e44,stroke:#237a35,color:#fff
    class C bad
...
- **코드와 데이터를 나누면** → 권한을 바꿀 때 **코드를 안 건드리고 JSON만** 고치면 됩니다.
- 그래서 **개발자가 아닌 보안 담당자도** 권한을 수정할 수 있습니다.
- 2과목에서 배운 "설정(config)을 분리한다" 원칙과 똑같습니다.

!!! question "확인질문"
    **Q. 역할·권한 데이터를 코드가 아니라 JSON 파일로 관리하면, 비개발자도 수정할 수 있을까요?**

    **A.** 네. JSON은 **사람이 읽고 고치기 쉬운 형식**이라, 코드를 몰라도 "이 역할에 이 권한을 추가"를 파일에서 바로 할 수 있습니다. 코드(로직)는 그대로 두고 데이터만 바뀌니 안전하고요.

<div class="quiz">
<p class="quiz-q"><span class="tag">퀴즈</span><b>역할·권한 데이터를 코드에 하드코딩하지 않고 JSON 파일로 분리하는 이유로 가장 적절한 것은?</b></p>
<button class="quiz-opt">JSON이 파이썬보다 실행 속도가 빠르기 때문</button>
<button class="quiz-opt" data-correct>권한을 바꿀 때 코드를 건드리지 않고 데이터 파일만 고치면 돼, 비개발자도 안전하게 수정할 수 있기 때문</button>
<button class="quiz-opt">JSON을 쓰면 권한이 자동으로 암호화되기 때문</button>
<button class="quiz-opt">코드에는 딕셔너리를 넣을 수 없기 때문</button>
<div class="quiz-explain"><b>정답: 2번.</b> '로직(코드)'과 '데이터(권한)'를 나누면 유지보수가 쉬워집니다. 속도(1번)·암호화(3번)와는 무관합니다.</div>
<button class="quiz-retry">다시 풀기</button>
</div>

---

## ⏱️ 95-120분 · 조회 CLI와 정리

!!! abstract "이 블록을 마치면"
    ✔ ==명령 한 줄로 권한을 확인하는 도구(CLI)==의 가치를 안다

`has_permission()`을 만들었어도, 매번 코드를 열어 실행하긴 불편합니다. 그래서 **CLI(명령줄 도구)**로 감쌉니다.

```text
$ python check_access.py --user kim01 --perm 고객정보_조회
→ 허용 ✅

$ python check_access.py --user kim01 --perm 급여_수정
→ 거부 ❌
```

파이썬의 `argparse`가 `--user`, `--perm` 같은 **입력값을 받아** 함수에 넘겨줍니다.

!!! tip "🎓 강사 뷰 · 왜 CLI가 중요한가"
    보안관제 담당자는 개발자가 아닙니다. 코드를 열지 않고 ==명령 한 줄로 "이 사람 이거 되나?"를 즉시 확인==할 수 있으면, 실무에서 훨씬 유용합니다. (실습에서 직접 만듭니다.)

!!! question "확인질문"
    **Q. 이런 CLI 도구가 있으면 보안관제 담당자가 매번 코드를 열어보지 않고도 무엇을 할 수 있을까요?**

    **A.** 명령 한 줄로 **"이 사용자가 이 권한을 가졌는지"를 바로 확인**할 수 있습니다. 코드를 몰라도, 파일을 안 열어도 되니 실무에서 빠르고 편합니다.

**정리:** 계정→역할→권한(RBAC)을 딕셔너리로 표현하고, `has_permission()`으로 확인, `assign_role()`로 부여, 데이터는 JSON으로 분리, CLI로 감쌌습니다. **오후(강의2)엔 "필요한 최소한"을 어떻게 기준으로 만드는지(권한 매트릭스·예외관리)**를 봅니다.
...
- [ ] `assign_role()`의 `setdefault`·중복 방지를 설명한다
- [ ] 데이터를 JSON으로 분리하는 이유를 설명한다
- [ ] CLI(argparse)가 왜 유용한지 설명한다
- [ ] 확인질문 5개 + 퀴즈에 답한다

*[RBAC]: Role-Based Access Control — 역할 기반 접근통제
*[PDP]: Policy Decision Point — 접근을 판단하는 정책 결정점(강의2에서 코드로 구현)
*[CLI]: Command Line Interface — 명령줄로 쓰는 도구
*[JSON]: JavaScript Object Notation — 키·값으로 데이터를 적는 형식
