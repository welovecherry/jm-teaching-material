...
!!! warning "이건 3·4·5과목용 '깊은 형식' 파일럿입니다"
    강사님이 3·4·5과목은 거의 처음이라, 2과목보다 **더 깊고·쉽고·자세하게** 만든 시험판입니다.
    새로 들어간 장치: 💻 **코드 한 줄씩 해부** · 📘🎓 **학습자/강사 이중 뷰** · 🧱 **파이썬 브릿지** · 🧠 **파인만 자가진단**.
    이 형식이 괜찮은지 봐주세요. (00-20 / 20-45 / 45-70분 = 코드가 처음 나오는 데까지)

> **이 교시 한 문장:** "누가 무엇에 접근할 수 있는가"를 **역할(Role)**로 묶어 관리하는 RBAC을, 개념부터 **파이썬 코드 한 줄까지** 이해합니다.
...
!!! info "📘 학습자 뷰 · 처음 보는 나"
    **접근통제(Access Control)** = "누가(사용자) 무엇에(자원) 접근할 수 있는지를 정하고 지키는 것"입니다.
    회사에 문서·시스템이 많은데 **아무나 다 열 수 있으면 위험**하죠. 그래서 "이 사람은 이것까지만"을 정합니다.

    한 가지 헷갈리기 쉬운 짝:
    - **인증(Authentication)** = "너 진짜 김철수 맞아?" → **누구인지 확인** (로그인·MFA)
    - **인가/접근통제(Authorization)** = "김철수는 이 문서를 열 수 있어?" → **무엇을 할 수 있나 확인**
...
!!! example "🎓 강사 뷰 · 가르치는 나"
    ...
    - **예상 질문 & 답:**
        ...
        - *"2과목에서 PDP/PEP 배웠는데 그거랑 뭐가 달라요?"* → 오늘 만드는 게 바로 **PDP를 코드로 구현**하는 겁니다(강의2에서 `evaluate_access()`로 연결).
...
!!! info "📘 학습자 뷰 · 처음 보는 나"
    네 단어부터 정리합니다.
    - **계정(Account)** = 누구 (예: `kim01`)
    - **권한(Permission)** = 무엇을 할 수 있나 (예: `고객정보_조회`)
    - **역할(Role)** = 권한을 묶은 꾸러미 (예: '영업담당자' = 고객정보_조회 + 견적서_작성)
    - **그룹(Group)** = 계정을 묶은 것 (예: '영업팀' = kim01, lee02 …)
...
!!! warning "🎓 강사 뷰 · 이 줄을 바꾸면? (자주 나오는 질문)"
    - **`user_roles.get(user, [])`를 `user_roles[user]`로 바꾸면?** → 없는 사용자를 조회할 때 **`KeyError`(에러)로 프로그램이 멈춥니다.** `.get`은 그걸 막아주는 안전장치입니다. → 학생이 `[]` 대괄호로 짜서 에러 내는 일이 흔하니 미리 짚어 주세요.
